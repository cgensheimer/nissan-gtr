...
async def get_courses(
    user_uuid: str, course_service: CourseService = Depends(get_course_service)
):
...
async def create_user(
    name: str, course_service: CourseService = Depends(get_course_service)
):
    ...
    return {
        ...
        "uuid": user_uuid,
        "ntfy_topic": new_user["ntfy_topic"],
    }
...
async def add_course_to_user(
    ...
    if add_crn_to_user(user_uuid, crn):
        ...
        raise HTTPException(
            status_code=404, detail="User not found or CRN already exists"
        )
