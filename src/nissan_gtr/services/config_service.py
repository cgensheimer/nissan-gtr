...
def add_user(name, password):
    ...
    ntfy_topic = (
        f"gt_registration_{base64.b64encode(name.encode()).decode().rstrip('=')}"
    )
    new_user = {
        user_uuid: {
            "name": name,
...
def remove_crn_from_user(user_uuid, crn):
    config = get_config()
    for user in config["Users"]:
        if user_uuid in user:
            if crn in user[user_uuid]["courses"]:
                user[user_uuid]["courses"].remove(crn)
                save_config(config)
                return True
    return False
...
def validate_user_password(user_uuid, password):
    user = get_user_by_uuid(user_uuid)
