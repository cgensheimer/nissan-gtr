...
class CourseService:
    ...
    async def check_single_course(self, course_key, course, user_uuid, logger):
        """Check a single course and return any messages to send"""
        try:
            await course.refresh_course_data()
            current_state = self.course_states[course_key]
            registration_info = await course.get_registration_info()
            ...
            if new_is_open:
                if new_seats_available <= 5 and current_state.seats_available > 5:
                    messages.append(
                        f"Only {new_seats_available} seats remaining for {course.name}"
                    )
                elif new_seats_available - current_state.seats_available >= 3:
                    messages.append(
                        f"{new_seats_available - current_state.seats_available} new seats opened for {course.name}"
                    )
                ...
                if new_waitlist_seats <= 5 and current_state.waitlist_seats > 5:
                    messages.append(
                        f"Only {new_waitlist_seats} waitlist spots remaining for {course.name}"
                    )
                elif new_waitlist_seats - current_state.waitlist_seats >= 3:
                    messages.append(
                        f"{new_waitlist_seats - current_state.waitlist_seats} new waitlist spots opened for {course.name}"
                    )
            ...
            # Send notifications for this course
            ...
            if logger:
                logger.error(f"Error checking course {course.crn}: {e}")
    ...
    async def check_courses(self, user_uuid):
        ...
        while True:
            ...
            # Create tasks for all courses to run in parallel
            tasks = []
            for course_key, course in user_courses.items():
                task = asyncio.create_task(
                    self.check_single_course(course_key, course, user_uuid, logger)
                )
                tasks.append(task)

            # Wait for all course checks to complete in parallel
            if tasks:
    ...
    async def start_user_task(self, user_uuid):
        ...
        if user_uuid not in self.user_tasks or self.user_tasks[user_uuid].done():
            self.user_tasks[user_uuid] = asyncio.create_task(
                self.check_courses(user_uuid)
            )
